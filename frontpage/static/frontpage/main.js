...
(() => {
   ...
   * =========================== */
    ...
    function initializeColourTomSelects(root = document) {
        if (!window.TomSelect) return;

        function renderColourOption(data, escape) {
        ...
        root.querySelectorAll("select.colour-tomselect").forEach((el) => {
            if (!el.tomselect) {
                new window.TomSelect(el, {
                    ...asOptions(el),
                    render: {
                        option: renderColourOption,
                        item: renderColourOption,
                    },
                });
            }
        });
        ...
        htmx.defineExtension("tomselect", {
            onEvent: function (name, evt) {
                if (name === "htmx:beforeSwap") {
                    ...
                    if (elt) {
                        elt.querySelectorAll("select.tomselect, select.colour-tomselect").forEach((el) => {
                ...
                if (name === "htmx:afterSettle") {
                    ...
                    if (elt) {
                        ...
                        initializeColourTomSelects(elt);
                    }
                }
            },
        });
